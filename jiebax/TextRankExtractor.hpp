...
namespace cppjieba {
  ...
  TextRankExtractor(const string& dictPath, 
    ...
    pos_tagger_(dictPath, hmmFilePath, userDict) {
  ...
  TextRankExtractor(const Jieba& jieba, const string& stopWordPath) : 
    ...
    // aaronyin-code-start
    void ExtractByWords(const vector<string>& words, vector<pair<string, double> >& keywords, size_t topN, size_t span=5, size_t rankTime=10) const {
      vector<Word> topWords;
      ExtractByWords_(words, topWords, topN, span, rankTime);
      for (size_t i = 0; i < topWords.size(); i++) {
        keywords.push_back(pair<string, double>(topWords[i].word, topWords[i].weight));
      }
    }

    void ExtractByWords_(const vector<string>& words, vector<Word>& keywords, size_t topN, size_t span=5, size_t rankTime=10) const {
    ...
    // aaronyin-code-start
      // void ExtractSents(const string& text, vector<string>& keysents, size_t topN, size_t span=5, size_t rankTime=10) const {

      //   // first cut text to words
      //   vector<string> words;
      //   segment_.Cut(text, words);

      //   // one pass of words, gen word_to_sent mapping
      //   for (vector<string>::iterator words_iter = words.begin(); words_iter != words.end(); ++words_iter)
      //   {
      //     // filter words
      //     if ()
      //     {
      ...
      //   TextRankExtractor::SentGraph graph;
      //   WordMap wordmap;
      //   size_t offset = 0;
