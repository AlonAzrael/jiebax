...
class Jieba {
 public:
  ...
  void TagNAV(const string& sentence, vector<pair<string, string> >& words) const {
    pos_tagger_.TagNAV(sentence, words);
  }
  void TagFilter(const string& sentence, vector<pair<string, string> >& res, vector<string>& res_no_pair, vector<string>& ifin_list, vector<string>& startswith_list, int return_pair = 0 ) const {
    pos_tagger_.TagFilter(sentence, res, res_no_pair, ifin_list, startswith_list, return_pair);
  }
